 ...
 */
 public class LorryListAdapter extends RecyclerViewCursorAdapter<LorryListAdapter.CustomerViewHolder>{
    private static final String TAG = LorryListAdapter.class.getSimpleName();
    private final Context mContext;
    public LorryListAdapter(Context context, Cursor cursor)
    {
        super(null);
        mContext = context;
    ...
    protected void onBindViewHolder(CustomerViewHolder holder, Cursor cursor)
    {
        ...
        String transportName = cursor.getString(cursor.getColumnIndex(MMDbHelper.TRANSPORTER_NAME));
        String lorryNo = cursor.getString(cursor.getColumnIndex(MMDbHelper.LORRY_NO));
        String srcAddress = cursor.getString(cursor.getColumnIndex(MMDbHelper.LORRY_SRC_ADDRESS));
        String pnoeNo = cursor.getString(cursor.getColumnIndex(MMDbHelper.LORRY_PHONE_NO));
        int sunDay = cursor.getInt(cursor.getColumnIndex(MMDbHelper.LORRY_COMES_ON_MON));
        int monDay = cursor.getInt(cursor.getColumnIndex(MMDbHelper.LORRY_COMES_ON_TUE));
        int tueDay = cursor.getInt(cursor.getColumnIndex(MMDbHelper.LORRY_COMES_ON_WED));
        int wedDay = cursor.getInt(cursor.getColumnIndex(MMDbHelper.LORRY_COMES_ON_THR));
        int thurDay = cursor.getInt(cursor.getColumnIndex(MMDbHelper.LORRY_COMES_ON_FRI));
        int friDay = cursor.getInt(cursor.getColumnIndex(MMDbHelper.LORRY_COMES_ON_SAT));
        int satDay = cursor.getInt(cursor.getColumnIndex(MMDbHelper.LORRY_COMES_ON_SUN));
        if(monDay == 1){
        ...
        holder.name.setText(transportName + " "+lorryNo);
        holder.description.setText(srcAddress);
